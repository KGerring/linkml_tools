...
1. A dashboard config YAML file (called `dashboard-config.yml` in the following)
...
1. `ontologies`: this is where ontologies are downloaded to and synced up
1. `sparql`: an optional directory that allows you to add custom checks on top of the usual OBO profile.
