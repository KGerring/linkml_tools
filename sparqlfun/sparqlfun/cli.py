...
@click.option(
    "-f", "--input-format", type=click.Choice(OutputFormat.list()), help="input format"
)
@click.option(
    "-O",
    "--to-format",
...
@click.option(
    "-P", "--prefix", multiple=True, help="prefix list, specified as prefix=uribase"
)
@click.option(
    "-m",
    "--module",
    default="sparqlfun.model",
    show_default=True,
    help="name of module to load",
)
@click.option("-S", "--schema", help="path to schema, if not default")
@click.option(
    "--yasgui/--no-yasgui",
    default=False,
    show_default=True,
...
@click.option("-T", "--template", required=True, help="name of template")
...
def query(
    ...
    if to_format == "tsv":
        dump_str = csv_dumper.dumps(
            container, index_slot="results", schemaview=se.schema_view
        )
    elif to_format == "ttl":
...
@main.command()
@click.option("--endpoint", "-e", help="Name or path of endpoint", required=False)
@click.option("--module", "-m", help="name of module")
@click.option(
    "-D",
    "--detail/--no-detail",
    default=False,
...
def templates(matches, endpoint, module, detail):
    ...
    for cn, c in sv.all_classes().items():
        ...
        if module:
            include = (
                module == this_mod
                or mmap.get(this_mod, Node) == module
                or mmap.get(module, None) == this_mod
            )
        ...
        if matches:
            if not any(m.lower() in cn.lower() for m in matches):
                logging.debug(
                    f"Skipping: {cn} Could not find any of the matches: {matches}"
                )
                continue
        if detail:
            print(f"\n## {cn}\n")
