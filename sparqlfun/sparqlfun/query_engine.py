...
class SparqlEngine:
    ...
    def query(
        self,
        template: Union[Type[YAMLRoot], YAMLRoot, SparqlTemplateInstance],
        _url=None,
        **kwargs,
    ) -> ResultSet:
        """
        ...
        if sq.query_type == "SELECT":
            ...
            if results_slot_av:
                ...
                logging.info(
                    f"result_slot {results_slot.name}, range = {results_slot_range}"
                )
            ...
            for obj in rdflib_loader.from_rdf_graph(
                g,
                target_class=result_template,
    ...
    def construct(self, template: str, _url=None, **kwargs) -> Graph:
        if _url is None:
            _url = self.get_endpoint().url
        logging.info(f"Querying: {_url}")
        prefix_map = self._get_prefix_map()
        # TODO: quote literals
        ti = SparqlTemplateInstance(
            query=SparqlQuery(template),
            bindings=kwargs,
    ...
    def extract_template_instance(
        self,
        template: Union[Type[YAMLRoot], YAMLRoot, SparqlTemplateInstance],
        **kwargs,
    ) -> SparqlTemplateInstance:
        """
        ...
        if isinstance(template, SparqlTemplateInstance):
            if len(kwargs.items()):
                logging.error(
                    f"Ignoring kwargs: {kwargs}, as template already instantiated: {template}"
                )
            return template
    ...
    def extract_sparql(self, template: Type[YAMLRoot], **kwargs) -> SparqlQuery:
        ...
        query = jt.render(_values=lambda l: " ".join(l), **kwargs)
    ...
    def get_config(self):
        if self.config is None:
            self.config = yaml_loader.load(
                config_path, target_class=SystemConfiguration
            )
    ...
    def _get_query_template(self, cls: ClassDefinition, kind="sparql.select") -> str:
        ...
        for rule in cls.classification_rules:
            if not rule.is_a:
                continue
            q = self._get_query_template(
                self.schema_view.get_class(rule.is_a), kind=kind
            )
            if q is None:
                return None
            d = {}
    ...
    def _get_prefix_map(self) -> PREFIXMAP:
        ...
        if self.prefix_map is None:
            self.prefix_map = {
                k: str(v) for k, v in self.schema_view.namespaces().items()
            }
    ...
    def _replace(self, template: str, argdict: dict = {}) -> str:
        ...
        m = re.search(
            r"(.*\s+where\s*\{)(\s+.*)", template, flags=re.IGNORECASE | re.DOTALL
        )
        if m:
            in_select = m.group(1)
